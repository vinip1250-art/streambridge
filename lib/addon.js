...
const { configHash } = require('./security');
...
function createAddon(rawConfig) {
  ...
  async function stream({ type, id }) {
    ...
    const configB64 = Buffer.from(JSON.stringify({ primary, secondary })).toString('base64url');
      ...
      try {
        ...
        if (inst.role === 'secondary') {
          if (owner === 'secondary') {
            ...
            nativeId = await resolveId(userKey, requestedId, primary, inst);
            if (!nativeId) {
          ...
          if (owner === 'secondary' && secondary) {
            nativeId = await resolveId(userKey, requestedId, secondary, inst);
            if (!nativeId) {
              if (DEBUG) console.log(`[stream] primary: item exclusivo do secondary sem cross-ref – pulando.`);
              continue;
            }
          } else {
            nativeId = requestedId;
        ...
        const [item, playbackInfo] = await Promise.all([
          ...
          getPlaybackInfo(inst, nativeId).catch(err => {
